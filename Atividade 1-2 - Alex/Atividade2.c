...
void inserirOrdenado(int *vetor, int *tamanho, int novoValor) {
    int i;
    for (i = 0; i < *tamanho; i++) {
...
int main() {
    ...
    int novoValor = 10;
    inserirOrdenado(vetorOrdenado, &tamanhoOrdenado, novoValor);
    printf("\nVetor Ordenado apos insercao de %d: ", novoValor);
    for (int i = 0; i < tamanhoOrdenado; i++) {
        printf("%d ", vetorOrdenado[i]);
    }
    printf("\n");
    ...
    printf("Elemento a ser procurado por busca binaria: %d\n", elemBin);
    indiceBinaria = buscaBinaria(vetorOrdenado, tamanhoOrdenado, elemBin);
    printf("A posicao do elemento no vetor e: %d\n", indiceBinaria);
